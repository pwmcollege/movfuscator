...
illustrates compiling a simple prime number function with gcc and the
M/o/Vfuscator.
...
The Docker image builds M/o/Vfuscator inside Ubuntu and installs `movcc` as the
entrypoint. Ubuntu-versioned images are published at `pwmcollege/movcc`.

```bash
docker pull pwmcollege/movcc:26.04
docker run --rm -v "$PWD:/work" pwmcollege/movcc:26.04 example.c -o example
```

Copy `movcc` into another Ubuntu image:
...
COPY --from=pwmcollege/movcc:26.04 /opt/movfuscator /opt/movfuscator
RUN ln -sfn /opt/movfuscator/build/movcc /usr/local/bin/movcc
```

Available Ubuntu tags:

- pwmcollege/movcc:20.04
- pwmcollege/movcc:22.04
...
```bash
docker build --build-arg UBUNTU_VERSION=26.04 -t pwmcollege/movcc:26.04 .
```

Open a shell in the image:

```bash
docker run --rm -it --entrypoint bash -v "$PWD:/work" pwmcollege/movcc:26.04
```
...
Flags are passed to the compiler proper via `-Wfflag`, for example
`-Wf--no-mov-id`.
...
The M/o/Vfuscator contains a complete mov-only floating point emulator. Since it
is approximately 500,000 instructions, you must explicitly link to it if you
need it. Use the smaller `softfloat32.o` if you only use floats,
`softfloat64.o` if you need floats and doubles, and `softfloatfull.o` for a
full IEEE floating point emulator.
...
Due to the lookup tables used for arithmetic, the symbol tables in the compiled
binary can be extremely large. Stripping symbols with the `-s` flag can cut the
final binary size by half.
...
You may occasionally run into ABI conflicts when calling into libraries compiled
with other compilers. These are relatively rare, but watch out for:
...
Such ABI issues can be roughly fixed with some coding caution, or more robustly
fixed by recompiling your libraries with the M/o/Vfuscator.

LCC is a C89 compiler. If you try to compile C99 code, you will get errors. Do
not use C99 features like `bool` or `for (int ...)`, and do not declare
variables in the middle of blocks.

To generate code correctly, the M/o/Vfuscator must be able to determine when a
call is being made to a shared library. If a function is used before it is
declared, this determination may be incorrect. If your executable hangs, it may
be because a function prototype was missing.

LCC strongly enforces type and const rules. You may not be able to compile code
with unsafe type or const conversions unless you explicitly cast for it.
...
When calling into libraries compiled with other compilers, an occasional
non-mov instruction is necessary. This can be fixed by compiling all of your
libraries with the M/o/Vfuscator. To be clear, this is not a limitation of the
M/o/Vfuscator, or a violation of mov Turing-completeness: non-mov instructions
are only necessary to accommodate non-mov calling conventions.

If you do not recompile your libraries, the non-mov instructions are as follows:

- A single `jmp` instruction at the end of the program is used to dispatch the
  program's external functions. To avoid this `jmp` instruction for a truly
  mov-only program, compile with `--no-mov-extern` and avoid calling into shared
  libraries. If you still need an external function, recompile the library with
  the M/o/Vfuscator and statically link it to your program.

- An `fstp` instruction is used following every call to an external function
  returning a float or a double. This is because there is no way to access the
  x86 floating point registers with a mov instruction. To get rid of the `fstp`
  instruction, recompile your libraries to use the M/o/Vfuscator's floating
  point emulator instead of the x86 FPU.
...
The M/o/Vfuscator currently uses C as the source language. The following proof
of concept, provided by Adam Schwalm, illustrates how to use a variety of other
source languages:
...
forms and addressing modes for the mov instruction. Although the point was never
anything different, the RISC post-processor (`risc.py`) illustrates transforming
the program into RISC-like data transfers, reducing the program to 4 byte memory
accesses, only indexed addressing modes, and only two registers.

With this, all mov instructions become either `mov esi/edi, [BASE+esi/edi]` or
`mov [BASE+esi/edi], esi/edi`, where `BASE` is some constant address. This
should demonstrate the applicability of the approach on non-x86 architectures,
and deter complaints about the diversity of the x86 mov instruction.

## Notes

While Dolan's paper required a `jmp` instruction, the M/o/Vfuscator does not: it
uses a faulting mov instruction to achieve the infinite execution loop. If
...
A `jmp` _is_ currently used to dispatch external functions. If this is a
problem, avoid using external functions, or compile libraries with the
M/o/Vfuscator as well.
